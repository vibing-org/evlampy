...
Можно отредактировать свой запрос или поретраить ответ LLM.
...
- `provider`: `"openai-compatible"` или `"codex"`.
- `models`: массив с названиями моделей, как у провайдера.
- `codexModels`: массив моделей Codex CLI, используется когда `provider` равен `"codex"`.
...
Чтобы использовать подписку Codex вместо API-ключа:
- Измените `provider` на "codex" в настройках `Evlampy: Open Global Config`
- Выполните `Evlampy: Sign in to Codex`, чтобы войти в свой аккаунт

Стоимость запросов в долларах не отображается при использовании подписки.
